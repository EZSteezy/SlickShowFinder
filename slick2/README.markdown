CURRENTLY A WORK IN PROGRESS. Still learning plenty and trying to take a tv show finder i made with jQuery and work it into using the Slick library to create a preview scroller. So far i've meshed (not cleaned up yet) the code for both my finder and the scroller. However, in doing so I broke what was emptying my body on submit clicks and refreshing results. Im also throwing 1-2 simple errors on the console due to needing to move some files around. The repo has a TON of files to get rid of due to non necessity, but this was about as far as I got before turning it off for the night.
